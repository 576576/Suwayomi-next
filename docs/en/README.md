...
cd jvm-sandbox
gradle build          # every Android stub is produced by the build, no jars to place:
                      #   AndroidCompat (with its Config module) is vendored source under
                      #   android-compat/; the AOSP public API shell is generated by
                      #   android-stub/ from the pin in android-stub.properties (the first
                      #   build downloads platform-30_r03.zip, about 52MB)
cd ..
