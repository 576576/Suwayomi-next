    ...
    pub fn serve_tcp_listener(&self, listener: TcpListener) -> Result<()> {
        ...
        for stream in listener.incoming() {
            let stream = match stream {
                Ok(s) => s,
                Err(err) => {
                    eprintln!("pglite proxy: accept error: {err}");
                    continue;
                }
            };
            // A single client error (connection abort, or any SQL error that
            // terminates the embedded session) must NOT take the whole proxy
            // down — otherwise every later pool acquire hangs forever.
            if let Err(err) = self.handle_stream(stream, &mut backend) {
                eprintln!("pglite proxy: connection error: {err:#}; continuing to accept");
            ...
            match listener.accept() {
                Ok((stream, _)) => {
                    if let Err(err) = stream.set_nonblocking(false) {
                        eprintln!("pglite proxy: configure stream: {err}");
                        continue;
                    }
                    // Do not let one bad client take the whole proxy down;
                    // see serve_tcp_listener for the rationale.
                    if let Err(err) = self.handle_stream(stream, &mut backend) {
                        eprintln!("pglite proxy: connection error: {err:#}; continuing to accept");
                        backend.rollback_connection_state();
                    }
                ...
                Err(err) => eprintln!("pglite proxy: accept error: {err}"),
            }
        }
