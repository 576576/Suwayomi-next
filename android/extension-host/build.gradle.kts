...
//
// 与桌面 jvm-sandbox 的关系（见 docs/migration/ANDROID_IMPL.md D2/D3）：
//  * **共享**：`../../extension-runtime/src/main/kotlin` —— `eu.kanade.tachiyomi.**`
...
// context parameters。AGP 9.2.1 内置的 Kotlin 是 **2.3.20**（jvm-sandbox 用的是
...
androidComponents {
    onVariants { variant ->
        variant.sources.kotlin?.addStaticSourceDirectory(
            file("../../extension-runtime/src/main/kotlin").absolutePath,
        )
    }
}
...
dependencies {
    ...
    // 编译期就需要的签名（extension-runtime 里也有 typealias），因此必须进依赖。
    implementation("androidx.preference:preference:1.2.1")
    implementation("androidx.annotation:annotation:1.9.1")

    // --- 扩展 API 运行时（与 jvm-sandbox 保持一致：扩展不自带第三方库，全部由宿主提供）---
