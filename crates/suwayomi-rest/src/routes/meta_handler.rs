...
pub async fn get_global_meta(s: &AppState) -> Result<HashMap<String, String>> {
    let rows = suwayomi_db::query("SELECT meta_key, value FROM global_meta").fetch_all(s.db.pool()).await?;
...
pub async fn set_global_meta(s: &AppState, key: String, value: String) -> Result<()> {
    ...
    let row = suwayomi_db::query(&existing).bind(&key).fetch_optional(s.db.pool()).await?;
    if let Some(row) = row {
        let id: i32 = row.try_get("id")?;
        let sql = bind_placeholders("UPDATE global_meta SET value = ? WHERE id = ?");
        suwayomi_db::query(&sql).bind(&value).bind(id).execute(s.db.pool()).await?;
    } else {
        let sql = bind_placeholders("INSERT INTO global_meta (meta_key, value) VALUES (?, ?)");
        suwayomi_db::query(&sql).bind(&key).bind(&value).execute(s.db.pool()).await?;
    }
    Ok(())
}
